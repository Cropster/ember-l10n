...
module.exports = Object.assign(BaseCommand, {
  ...
  _generateOnly(options) {
    ...
    let potFile = this._makePath(options.extractTo, options.generateFrom);
  ...
  _makePOTFiles(options) {
    this.potExcludeFile = this._makePath(this.tmpFolder, 'excluded.pot');
    this.potFile = this._makePath(this.tmpFolder, options.potName);
  ...
  _parseFolder(basePath, options) {
    ...
    modules.forEach((module) => {
      ...
      let modulePath = this._makePath(basePath, module);
      let stat = fs.statSync(modulePath);
      if (!stat.isDirectory()) {
        return;
      }

      let json = this._makePath(modulePath, 'package.json');
      ...
      options.extractFrom.push(this._makePath(modulePath, 'addon'));
    });
  },
...
========================================
    ...
    files.forEach((file) => {
      ...
      let parser = this._makePath('./node_modules', 'xgettext-template', 'bin', 'xgettext-template');
      ...
      shell.exec(`node ${parser} ${nodeArgs}`);
  ...
  _initPOTFile(options, inputPOTFile) {
    ...
    outputPOTFile = this._makePath(options.extractTo, outputPOTFile);
...
========================================
  ...
  _initPOFile(options, potFile, poFile) {
    ...
    poFile = this._makePath(options.extractTo, poFile);

...
let fs = require('fs');
let { parseHbsFile } = require('./utils/parse-hbs');
let { buildPotFile } = require('./utils/build-pot-file');
let { mergePotFiles } = require('./utils/merge-pot-files');
...
========================================
    ...
    files.forEach((file) => {
      ...
      parseHbsFile(file, options, isExcluded ? excludedItems : messageItems);
    });

    let messagesPotContent = buildPotFile(messageItems, options);
    let excludedPotContent = buildPotFile(excludedItems, options);

    // create temporary files
    let tmpMessagesPotFile = path.join(options.extractTo, 'messages.pot.tmp');
    let tmpExcludedPotFile = path.join(options.extractTo, 'excluded.pot.tmp');

    shell.touch(tmpMessagesPotFile);
    shell.touch(tmpExcludedPotFile);

    fs.writeFileSync(tmpMessagesPotFile, messagesPotContent, encoding);
    fs.writeFileSync(tmpExcludedPotFile, excludedPotContent, encoding);

    // Now merge the tmp. files with the existing ones
    mergePotFiles(tmpMessagesPotFile, this.potFile, options);

    if (excludedItems.length) {
      mergePotFiles(tmpExcludedPotFile, this.potExcludeFile, options);
    }

    // cleanup temporary files{
    shell.rm('-f', [tmpMessagesPotFile, tmpExcludedPotFile]);

...
module.exports = Object.assign(BaseCommand, {
  ...
  _extractFromJS(options) {
    ...
    let files = directories.reduce((files, dir) => {
      try {
        let result = shell.find('-R', `${dir}/**/*.{js,ts}`);
        if (result.code === 0) {
...
========================================
    ...
    files.forEach((file) => {
      ...
      // Fix paths for Windows - there, xgettext-template makes paths like ./app\components\...
      let fileContent = fs.readFileSync(tmpPOTFile, encoding);

      let regex = /#: (.*)(\\)(.*)\.hbs/gm;
      while (regex.exec(fileContent)) {
        fileContent = fileContent.replace(regex, '#: $1/$3.hbs');
      }

     fs.writeFileSync(tmpPOTFile, fileContent);

...
module.exports = {
   ...
   */
  run(options) {
    let promise = this.checkDependencies_(options);
   ...
   * @return {Promise}
   */
  checkDependencies_(options) {
    let promiseCallback = (resolve, reject) => {
      if (options.skipCheckInstall || !this._hasMissingDependencies()) {
        resolve();
        return;
      }
  ...
  /**
   * Make a cross platform path from one or multiple path segments.
   * This takes one or multiple strings as arguments.
   *
   * @method _makePath
   * @return {String}
   * @private
   */
  _makePath() {
    let fullPath = Array.prototype.slice.call(arguments).join('/');
    if (fullPath.indexOf('.') === 0) {
      return `.${path.join(fullPath.substr(1))}`; // Remove the dot, parse it through path.join(), and then add the dot again
    }

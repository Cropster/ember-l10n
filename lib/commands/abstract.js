...
module.exports = {
  ...
  checkDependencies_() {
    let promiseCallback = (resolve, reject) => {
      ...
      this.ui.writeLine(chalk.red.bold(`You have to install missing dependencies for ember-l10n CLI!`));
   ...
   */
  hasGettext_() {
    let task = this.tryInvoke_(() => shell.exec('gettext --version', { silent: true }));
  ...
  hasNpmPackage_(pkg) {
    try {
      ...
      this.ui.writeLine(chalk.red.bold(`ERROR: ${pkg} npm package is not installed`));
      return false;
    }

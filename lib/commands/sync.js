...
========================================
    ...
    let files = this.tryInvoke_(() =>
      shell.find('-R', directories.map((dir) => `${dir}/**/*.{js,hbs}`))
    );
  ...
  _replaceFile(file, regex, msgid, msgstr, msgidPlural, msgstrPlural) {
    if (
      this._hasNoChangedValues({ msgid, msgstr, msgidPlural, msgstrPlural })
    ) {
      return 0;
    }
  ...
  _hasNoChangedValues({ msgid, msgstr, msgidPlural, msgstrPlural }) {
    msgid = (msgid || '').replace(/\s+/g, ' ');
    msgstr = (msgstr || '').replace(/\s+/g, ' ');
    msgidPlural = (msgidPlural || '').replace(/\s+/g, ' ');
    msgstrPlural = (msgstrPlural || '').replace(/\s+/g, ' ');
    return msgid === msgstr && msgidPlural === msgstrPlural;
  },

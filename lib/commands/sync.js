...
module.exports = {
  ...
  start: function(options) {
    this.poFile = `${options.inputDirectory}/${options.language}.po`;
...
========================================
    ...
    let directories = typeof options.outputDirectories==='string' ?
      [ options.outputDirectories ] :
      options.outputDirectories;
  ...
  replaceJS: function(file,key,msgid,msgstr,msgidPlural,msgstrPlural) {
    ...
    var regex = new RegExp(
      ...
      "\\\((?:\\\s*)(?:'|\\\"))" +                          // = $1
      "(" + this.escapeString(msgid) + ")" +                // = $2
      "('|\\\")" +                                          // = $3
      "(?:" +
        "(\\\s*,\\\s*(?:'|\\\"))" +                         // = $4
        "(" + this.escapeString(msgidPlural||"") + ")" +    // = $5
        "('|\\\")" +                                        // = $6
      ")?",
      "g"
    );

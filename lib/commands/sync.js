...
========================================
  ...
  extractMessages: function() {
    ...
    // important: parse PO file with `--no-wrap`
    // command, otherwise the above regex won't
    // work and we are missing lots of messages
    let command = shell.exec(
      `msgattrib --no-wrap ${this.poFile}`,
      {
        silent: true // no console output
      }
    );

    let data = command.stdout || '';
  ...
  escapeCharacter: function(string, character) {
  ...
  replaceJS: function(file,key,msgid,msgstr,msgidPlural,msgstrPlural) {
    ...
    var regex = new RegExp(
      ...
      "\\((?:\\s*)(?:'|\"))" +                                // = $1
      "(" + this.escapeForRegex(msgid) + ")" +                // = $2
      "('|\")" +                                              // = $3
      "(?:" +
        "(\\s*,\\s*(?:'|\"))" +                               // = $4
        "(" + this.escapeForRegex(msgidPlural||"") + ")" +    // = $5
        "('|\")" +                                            // = $6
      ")?",
      "g"
    );
  ...
  replaceFile: function(file,regex,msgid,msgstr,msgidPlural,msgstrPlural) {
    ...
    // important: remove line breaks etc. for
    // doing lookups - otherwise message ids
    // may not be found be the regex matching
    let contents = input.replace(/\s+/g, ' ');
    if (contents.match(regex)===null) {
      return -1;
    }

    // now convert msgstr to msgid in content string
    let replace = (match,p1,p2,p3,p4,p5,p6) => {
      let quotes = p6 || p3; // "|'

      return [
        p1,
        this.escapeCharacter(msgstr, quotes),
        p3,
        p4,
        this.escapeCharacter(msgstrPlural, quotes),
        p6
      ].join('');
    };

    let output = input.replace(regex, replace);

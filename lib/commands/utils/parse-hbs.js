...
function getItemText(item, fileName) {
  ...
  throw new Error(
    `You need to pass a string as argument to l10n helpers: ${fileName}:${item.loc.start.line}:${item.loc.start.column}`
  );
}
...
function parseHbs(fileName, astBody, gettextItems = []) {
  ...
  filterAstItemsByPath(filteredItems, ['t']).forEach((item) => {
    if (item.params.length < 1) {
      throw new Error(
        `t-helper does not seem to have proper arguments: ${fileName}:${item.loc.start.line}:${item.loc.start.column}`
      );
    }
  ...
  filterAstItemsByPath(filteredItems, ['pt']).forEach((item) => {
    if (item.params.length < 2) {
      throw new Error(
        `pt-helper does not seem to have proper arguments: ${fileName}:${item.loc.start.line}:${item.loc.start.column}`
      );
    }
  ...
  filterAstItemsByPath(filteredItems, ['n']).forEach((item) => {
    if (item.params.length < 3) {
      throw new Error(
        `n-helper does not seem to have proper arguments: ${fileName}:${item.loc.start.line}:${item.loc.start.column}`
      );
    }
  ...
  filterAstItemsByPath(filteredItems, ['pn']).forEach((item) => {
    if (item.params.length < 4) {
      throw new Error(
        `pn-helper does not seem to have proper arguments: ${fileName}:${item.loc.start.line}:${item.loc.start.column}`
      );
    }

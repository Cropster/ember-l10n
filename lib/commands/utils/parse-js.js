...
function getItemText(item, fileName) {
  ...
  if (item.type === 'TemplateLiteral') {
    if (item.quasis.length > 1) {
      throw new Error(
        `You should not use a template literal with variables inside - use l10n placeholders instead: ${fileName}:${
          item.loc.start.line
  ...
  throw new Error(
    `You need to pass a string as argument to l10n methods: ${fileName}:${
      item.loc.start.line
    }:${item.loc.start.column}`
  );
}

...
function groupGettextItems(gettextItems) {
  return gettextItems
    .sort((item1, item2) => {
      return item1.loc.fileName.localeCompare(item2.loc.fileName) || item1.loc.line - item2.loc.line;
    })
    .reduce((allGroups, item) => {
      let group = allGroups.find((group) => {
        return group.messageId === item.messageId
...
function buildPotFile(gettextItems, options) {
  ...
  return items.map((item) => {
    ...
    if (item.messageContext) {
      parts.push(buildPair('msgctxt', normalizeMsgId(item.messageContext)));
    }
    parts.push(buildPair('msgid', normalizeMsgId(item.messageId)));

    if (item.messageIdPlural) {
      parts.push(buildPair('msgid_plural', normalizeMsgId(item.messageIdPlural)));
    }
    ...
    if (item.messageIdPlural) {
      ...
      parts.push(buildPair('msgstr', normalizeMsgId(item.messageString)));
    }

...
export default Ember.Service.extend(Ember.Evented, {
	...
	setLocale(locale) {
		...
		console.info(`i18n.js: Locale set to: "${locale}"`);
		this.get("_i18n").setLocale(locale);
	...
	hasLocale(locale) {
		...
		if (!hasLocale) {
			console.warn(`i18n.js: Locale "${locale}" is not available!`);
		}
	...
	detectLocale() {
		...
		if (!this.hasLocale(locale)) {
			console.info(`i18n.js: Falling back to default language: "${defaultLocale}"!`);
			return defaultLocale;
		}

		// otherwise return detected locale
		if (Ember.isNone(forceLocale)) {
			console.info(`i18n.js: Automatically detected user language: "${locale}"`);
		} else {
			console.info(`i18n.js: Using forced locale: "${locale}"`);
		}
	...
	t(msgid,hash = {}) {
		if (Ember.typeOf(msgid)!=='string') {
			try{
				...
				console.log('i18n.js: "msgid" param for t() should be either a string or an object implementing toString() method!');
				return msgid;
			}
		}

		return this._strfmt(this.get('_i18n').gettext(msgid),hash);
	},
	...
	n(msgid,msgid_plural,count = 1,hash = {}) {
		if (Ember.typeOf(msgid)!=='string') {
			try{
				...
				console.log('i18n.js: "msgid" param for n() should be either a string or an object implementing toString() method!');
				return msgid;
			}
		}
		...
		if (Ember.typeOf(msgid_plural)!=='string') {
			try{
				...
				console.log('i18n.js: "msgid_plural" param for n() should be either a string or an object implementing toString() method!');
				return msgid;
			}
		}

		return this._strfmt(this.get('_i18n').ngettext(msgid,msgid_plural,count),hash);
	},
	...
	_loadJSON(){
		...
		const successCallback = response => {
			...
			this.get('_i18n').loadJSON(response);
			this.trigger('translation_loaded');
			cache[locale] = cachedResponse;
		};

		const failureCallback = reason => {
			console.error(`i18n.js: An error occured loading "${url}": ${reason}`);
		};

...
module('Unit | Service | l10n', function(hooks) {
  ...
  test('it works', async function(assert) {
    ...
    assert.strictEqual(
      service.t("I'm a {{placeholder}}.", { placeholder: 'rockstar' }),
      "I'm a rockstar.",
    ...
    assert.strictEqual(
      service.tVar("I'm a {{placeholder}}.", { placeholder: 'rockstar' }),
      "I'm a rockstar.",
    ...
    assert.strictEqual(
      service.exists('en'),
      true,
      'exists() delivers correct state for existing keys.'
    );
    assert.strictEqual(
      service.exists('de'),
      false,
      'exists() delivers correct state for non-existing keys.'
    );
  });
  ...
  test('detect and swap locale test', async function(assert) {
    ...
    assert.strictEqual(
      service.detectLocale(),
      'de',
      '`defaultLocale` is used if not in `availableLocales`.'
    );
    ...
    assert.strictEqual(
      service.getLocale(),
      'en',
      '`defaultLocale` is provided if no locale set.'
    );
    ...
    assert.strictEqual(
      service.getLocale(),
      'de',
      'Changing locales via `setLocale()` changes for supported locales.'
    );
    assert.strictEqual(
      service.t('testing'),
      'Test',
      'Changing locales via `setLocale()` loads corresponding translations.'
    );
  });
  ...
  test('_getBrowserLocales works', function(assert) {
    ...
    assert.deepEqual(
      service._getBrowserLocales(),
      ['de-AT', 'de', 'en-US', 'en'],
      'it uses the languages if found'
    );
  });
